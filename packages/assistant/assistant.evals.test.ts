...
const CASES: EvalCase[] = [
  ...
  },
  {
    ...
    utterance: 'Me pagaron un proyecto grande y ya puse 600 soles. ¿Cómo queda el plan?',
    asOf: '2026-09-01',
    extract: {
      kind: 'extraction',
      extraction: {
  ...
  },
  {
    ...
    utterance: 'Estos meses fueron flojos, solo sumé 50 soles más. Recalculame el plan.',
    asOf: '2026-12-01',
    extract: {
      kind: 'extraction',
      extraction: {
...
describe('evals: toda cifra de la respuesta tiene respaldo exacto del motor', () => {
  it.each(CASES)('$name', async (evalCase) => {
    ...
    const allowed = authorizedTokens(responses, 'PEN');
